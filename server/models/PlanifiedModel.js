...
class planifiedModel {
  ...
  static FormatProgram = (data, type = "all") => {
    ...
    const getFacultyAndClassIndex = (faculty, classCode) => {
      ...
      const facIndex = programsByFaculties.findIndex(
        (prog) => prog.facultyName === faculty
      );

      if (facIndex > -1) {
        // Get index of class
        const classIndex = programsByFaculties[facIndex].classes.findIndex(
          (myClass) => myClass.code === classCode
        );
    ...
    for (let prog of data) {
      ...
      const { facIndex, classIndex } = getFacultyAndClassIndex(
        prog.nomFil,
        prog.codeClasse
      );
      ...
      if (facIndex !== undefined && classIndex !== undefined) {
        ...
        programsByFaculties[facIndex].classes[classIndex].programs[
          program.day
        ].push(program);
      } else {
  ...
  static getProgramsByFaculty = async (payload) => {
    const { idAnneeAca, idSemestre, idFiliere } = payload;
  ...
  static getProgramByTeacher = async (payload) => {
    const { idAnneeAca, idSemestre, matriculeEns } = payload;
  ...
  static getProgramByRoom = async (payload) => {
    const { idAnneeAca, idSemestre, idSalle } = payload;
  ...
  static createProgram = async (payload) => {
    ...
    try {
      ...
      if (rows) {
        const { response, data } = await FollowModel.create({
          codeCours,
          idGroupe,
        });
  ...
  static updateProgram = async (payload) => {
    const { idAdmin, codeCours, idSalle, idJour, heureDebut } = payload.key;

    const { newHeureDebut, newHeureFin } = payload.data;
  ...
  static getProgram = async (payload) => {
    const { idAdmin, codeCours, idSalle, idJour, heureDebut } = payload;
