...
const { EXPIRE_IN, SECRET_CODE_TOKEN } = process.env;

class AdminController {
  static getCurrentUser(req, res) {
    return res
      .status(200)
      .json({ data: { ...req.user, passwordAdmin: undefined } });
  }

  static async getAll(req, res) {
    const { data, error } = await AdminModel.getAll();
  ...
  static async create(req, res) {
    // Get data from body
    const { name, password, email, phone, sexe } = req.body;
    ...
    if (email && password) {
      ...
      if (data) {
        ...
        if (verification) {
          ...
          return res
            .status(200)
            .json({ data: { ...data, passwordAdmin: undefined, token } });
        }
