...
class PlanifiedController {
  ...
  static getAllPrograms = async (req, res) => {
    ...
    if (!idAnneeAca || !idSemestre)
      return res.status(400).json({ error: "Provide all required data" });

    const { data, error } = await PlanifiedModel.getAllPrograms({
      idAnneeAca: Number(idAnneeAca),
  ...
  static getProgramsByFaculty = async (req, res) => {
    const { idAnneeAca, idSemestre, idFiliere } = req.params;

    if (idAnneeAca && idSemestre && idFiliere) {
      const { data, error } = await PlanifiedModel.getProgramsByFaculty(
        req.params
      );
  ...
  static getProgramByTeacher = async (req, res) => {
    const { idAnneeAca, idSemestre, matriculeEns } = req.params;

    if (!idAnneeAca || !idSemestre || !matriculeEns)
      return res.status(400).json({ error: "Provide all the required data" });

    const { data, error } = await PlanifiedModel.getProgramByTeacher(
      req.params
    );
  ...
  static getProgramByRoom = async (req, res) => {
    const { idAnneeAca, idSemestre, idSalle } = req.params;
  ...
  static createProgram = async (req, res) => {
    ...
    if (!checkData)
      return res
        .status(400)
        .json({ error: "Provide all informations datas to create the course" });
  ...
  static deleteProgram = async (req, res) => {
    ...
    if (!checkData)
      return res.status(400).json({
        error: "Provide all the required data to delete this program",
      });
  ...
  static updateProgram = async (req, res) => {
    ...
    const { newHeureDebut, newHeureFin } = req.body.data;
    ...
    if (!checkData)
      return res.status(400).json({
        error: "Provide all the required data to update this program",
      });
  ...
  static getProgramByDay = (allPrograms, day) => {
    ...
    for (let program of allPrograms) {
      if (program.nomJour.toLowerCase() === day.toLocaleLowerCase())
        programByDay.push(program);
    }
